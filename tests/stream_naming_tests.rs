//! Tests for EventCore stream naming conventions
//!
//! These tests verify that our stream naming factory functions produce
//! consistent, type-safe stream IDs for use with EventCore.

#[cfg(test)]
mod stream_naming_tests {
    use union_square::domain::streams;
    use union_square::domain::{AnalysisId, ExtractionId, SessionId, UserId};
    ...
    fn session_stream_creates_correct_format() {
        ...
        // Act
        let stream_id = streams::session_stream(&session_id);
    ...
    fn analysis_stream_creates_correct_format() {
        ...
        // Act
        let stream_id = streams::analysis_stream(&analysis_id);
    ...
    fn user_settings_stream_creates_correct_format() {
        ...
        // Act
        let stream_id = streams::user_settings_stream(&user_id);
    ...
    fn extraction_stream_creates_correct_format() {
        ...
        // Act
        let stream_id = streams::extraction_stream(&extraction_id);
    ...
    fn stream_ids_are_unique_for_different_entities() {
        ...
        // Assert
        assert_ne!(
            session_stream.as_ref(),
            analysis_stream.as_ref(),
    ...
    fn stream_ids_are_consistent_for_same_entity() {
        ...
        // Act
        let stream1 = streams::session_stream(&session_id);
        let stream2 = streams::session_stream(&session_id);
    ...
    fn stream_factory_functions_return_valid_eventcore_stream_ids() {
        ...
        let session_stream = streams::session_stream(&session_id);
        let analysis_stream = streams::analysis_stream(&analysis_id);
        let user_stream = streams::user_settings_stream(&user_id);
        let extraction_stream = streams::extraction_stream(&extraction_id);

        // Verify they can be used with EventCore APIs
        // (this is really just checking they're the right type)
        assert_eq!(session_stream, session_stream.clone());
        assert_eq!(analysis_stream, analysis_stream.clone());
        assert_eq!(user_stream, user_stream.clone());
        assert_eq!(extraction_stream, extraction_stream.clone());
    }
    ...
    fn stream_ids_are_deterministic() {
        ...
        let session_id = SessionId::generate();
        let stream = streams::session_stream(&session_id);
    ...
    fn stream_parsing_from_string_not_yet_implemented() {
        ...
        // let (stream_type, entity_id) = streams::parse_stream_id(&stream)?;
    ...
    fn stream_ids_follow_naming_convention() {
        ...
        // Verify prefixes
        assert!(
            session_stream.as_ref().starts_with("session:"),
            "Session streams must start with 'session:' prefix"
        );
        assert!(
            analysis_stream.as_ref().starts_with("analysis:"),
            "Analysis streams must start with 'analysis:' prefix"
        );
        assert!(
            user_stream.as_ref().starts_with("user:")
                && user_stream.as_ref().ends_with(":settings"),
            "User settings streams must follow 'user:{{id}}:settings' pattern"
        );
        assert!(
            extraction_stream.as_ref().starts_with("extraction:"),
