...
During the architecture alignment initiative, existing event schemas may be replaced when that is the cleanest path. After a schema is accepted as part of the aligned architecture, event evolution must be additive or use a new event variant so historical replay remains safe.
